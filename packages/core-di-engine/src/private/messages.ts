...
export const dependencyCycleShadowed = (names: readonly string[]): string => `${dependencyCycle(names)} (through a registration shadowed for resolve(); reachable via resolveAll(), which walks every registration)`;
