...
export const scopeMismatchUnderSingleton = (ownerName: string | undefined, tokenName: string, lifetime: Lifetime): string =>
  `${ownerName} (${asWord(lifetime)}) depends on ${tokenName}, which only a scope can serve, and is reached from a singleton: a singleton resolves at the root, so everything under it does too and no scope is ever available`;
