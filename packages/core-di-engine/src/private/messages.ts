...
export const lifetimeAfterCommit = (lifetime: Lifetime): string =>
  `This registration was already committed with the default lifetime (${asWord(lifetime)}) when its provider or scope was built. Call lifetime verbs before building or resolving.`;
...
export const scopeMismatchRootReachable = (ownerName: string | undefined, tokenName: string, lifetime: Lifetime): string =>
  `${ownerName} (${asWord(lifetime)}) depends on ${tokenName}, which only a scope can serve: resolved from the root it receives the root provider, which is not a scope`;
export const captiveDependency = (ownerName: string | undefined, depName: string | undefined, lifetime: Lifetime): string =>
  `${ownerName} (singleton) captures ${depName} (${asWord(lifetime)}) in its dependency tree, a captive dependency`;
