...
// throwing, so a plain `await import('@shellicar/core-di')` inside a vitest
// test cannot see this bug at all. A real `node` process, importing by the
// package's own name (going through its actual `package.json` "exports" map,
// the same path a real consumer takes), is the only thing that reproduces it.
